...
type Min15 struct {
	Code      string    `json:"code" xorm:"not null pk unique(pk_tbl_primary_min15) VARCHAR(20)"`
	Datetime  time.Time `json:"datetime" xorm:"not null pk unique(pk_tbl_primary_min15) DATETIME"`
	...
	Type      int       `json:"type" xorm:"not null pk unique(pk_tbl_primary_min15) INTEGER"`
}
func (k *Min15)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Min15{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Min15{})
		if err != nil {
			log.Logger.Debug("初始化Min15表失败!")
			return
		}
	}
}