...
type Min30 struct {
	Code      string    `json:"code" xorm:"not null pk unique(pk_tbl_primary_min30) VARCHAR(20)"`
	Datetime  time.Time `json:"datetime" xorm:"not null pk unique(pk_tbl_primary_min30) DATETIME"`
	...
	Type      int       `json:"type" xorm:"not null pk unique(pk_tbl_primary_min30) INTEGER"`
}
func (k *Min30)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Min30{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Min30{})
		if err != nil {
			log.Logger.Debug("初始化Min30表失败!")
			return
		}
	}
}