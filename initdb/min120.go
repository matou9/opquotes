...
type Min120 struct {
	Code      string    `json:"code" xorm:"not null pk unique(pk_tbl_primary_min120) VARCHAR(20)"`
	Datetime  time.Time `json:"datetime" xorm:"not null pk unique(pk_tbl_primary_min120) DATETIME"`
	...
	Type      int       `json:"type" xorm:"not null pk unique(pk_tbl_primary_min120) INTEGER"`
}
func (k *Min120)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Min120{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Min120{})
		if err != nil {
			log.Logger.Debug("初始化Min120表失败!")
			return
		}
	}
}