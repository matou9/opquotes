...
type Min5 struct {
	Code      string    `json:"code" xorm:"not null pk unique(pk_tbl_primary_min5) VARCHAR(20)"`
	Datetime  time.Time `json:"datetime" xorm:"not null pk unique(pk_tbl_primary_min5) DATETIME"`
	...
	Type      int       `json:"type" xorm:"not null pk unique(pk_tbl_primary_min5) INTEGER"`
}
func (k *Min5)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Min5{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Min5{})
		if err != nil {
			log.Logger.Debug("初始化Min5表失败!")
			return
		}
	}
}