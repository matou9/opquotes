...
type Min60 struct {
	Code      string    `json:"code" xorm:"not null pk unique(pk_tbl_primary_min60) VARCHAR(20)"`
	Datetime  time.Time `json:"datetime" xorm:"not null pk unique(pk_tbl_primary_min60) DATETIME"`
	...
	Type      int       `json:"type" xorm:"not null pk unique(pk_tbl_primary_min60) INTEGER"`
}
func (k *Min60)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Min60{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Min60{})
		if err != nil {
			log.Logger.Debug("初始化Min60表失败!")
			return
		}
	}
}