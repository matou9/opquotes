...
type Subtrade struct {
	Accountid       int64     `json:"accountid" xorm:"not null pk unique(pk_tbl_primary_subtrade) BIGINT"`
	...
	Tradeid         string    `json:"tradeid" xorm:"not null pk unique(pk_tbl_primary_subtrade) VARCHAR(100)"`
...
func (s *Subtrade)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Subtrade{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Subtrade{})
		if err != nil {
			log.Logger.Debug("初始化Subtrade表失败!")
			return
		}
	}
}