...
type Day struct {
	Code      string    `json:"code" xorm:"not null pk unique(pk_tbl_primary_day) VARCHAR(20)"`
	Datetime  time.Time `json:"datetime" xorm:"not null pk unique(pk_tbl_primary_day) DATETIME"`
	...
	Type      int       `json:"type" xorm:"not null pk unique(pk_tbl_primary_day) INTEGER"`
}
func (k *Day)CreateTable(){
	if ok,err:=pub.DB.IsTableExist(&Day{});!ok &&err==nil{
		err := pub.DB.CreateTables(&Day{})
		if err != nil {
			log.Logger.Debug("初始化Day表失败!")
			return
		}
	}
}