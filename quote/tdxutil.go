...
func ToKlineArr(l []*SecurityBar) ([]*Kline, error) {
	var ret []*Kline
	for _, sb := range l {
		k := &Kline{}
		k.Open = sb.Open
...
func ToTdxPeriod(src PeriodType) TdxKlineType {
	switch src {
	case PeriodType_M1:
...
func toDepthsMarketDataArr(l []*SecurityQuote) ([]*MarketDataSnapshot, error) {
	var ret []*MarketDataSnapshot
	for _, q := range l {
		var s MarketDataSnapshot
		s.Symbol=q.Code
		switch q.Market{
		case 1:
			s.Exchange="SH"
